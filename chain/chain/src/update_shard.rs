...
///
/// `cancel`, if `Some`, is the per-job cancellation handle flipped by the
/// chain (or by `ChunkExecutorActor` in SPICE mode) when the prev_block
/// memtrie root this update depends on is about to be pruned. The runtime
/// polls it and returns a recoverable error rather than panicking. Pass
/// `None` from paths that don't register cancellation (state-viewer tools,
/// tests).
