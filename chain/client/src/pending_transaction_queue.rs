...
impl PendingTransactionQueue {
    ...
    pub fn remove_certified_chunk(&mut self, block_hash: &CryptoHash) {
...
impl PendingTxSession {
    ...
    pub fn check_pending(
        ...
        if has_contract && !is_gas_key_tx && total_access_key_count >= P_MAX {
            return PendingTxCheckResult::Skip;
        }
...
mod tests {
    ...
    fn test_add_and_remove_chunk() {
        ...
        with_shard_ptq(&sharded, |ptq| ptq.remove_certified_chunk(&block_hash));
    ...
    fn test_incremental_chunk_removal() {
        ...
        with_shard_ptq(&sharded, |ptq| ptq.remove_certified_chunk(&hash1));
        with_shard_ptq(&sharded, |ptq| {
            assert_eq!(
                ptq.pending_accounts.get(&signer.get_account_id()).unwrap().access_key_tx_count,
                1
        ...
        with_shard_ptq(&sharded, |ptq| ptq.remove_certified_chunk(&hash2));
        with_shard_ptq(&sharded, |ptq| assert!(ptq.is_empty()));
    }
    ...
    fn test_session_p_max_enforcement() {
        ...
        assert_eq!(session.check_pending(&next_tx, true), PendingTxCheckResult::Skip);

        // But from a non-contract account it should be admitted.
        let mut session2 = make_session(&sharded);
        assert!(matches!(session2.check_pending(&next_tx, false), PendingTxCheckResult::Admit(_)));
    }
    ...
    fn test_deploy_exclusivity_pending_deploy_blocks_new_access_key_tx() {
        ...
        assert_eq!(session.check_pending(&transfer_tx, false), PendingTxCheckResult::Skip);
    }
    ...
    fn test_deploy_exclusivity_pending_access_key_tx_blocks_deploy() {
        ...
        assert_eq!(session.check_pending(&deploy_tx, false), PendingTxCheckResult::Skip);
    }
    ...
    fn test_deploy_exclusivity_gas_key_deploy_blocks_access_key_tx() {
        ...
        assert!(matches!(
            session.check_pending(&gas_key_transfer, false),
    ...
    fn test_session_accumulates_across_calls() {
        ...
        for i in 1..=P_MAX {
            ...
            assert!(
                matches!(session.check_pending(&tx, true), PendingTxCheckResult::Admit(_)),
        ...
        assert_eq!(session.check_pending(&tx, true), PendingTxCheckResult::Skip);
    }
    ...
    fn test_constraints_include_pending_balance() {
        ...
        assert_eq!(
            session.check_pending(&next_tx, false),
            PendingTxCheckResult::Admit(PendingConstraints {
